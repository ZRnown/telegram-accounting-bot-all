...
/**
 * 🔥 获取币安P2P详细价格信息（OTC价格）- 增强错误处理和超时控制
...
bot.hears(/^(OTC价格|查询OTC|币安价格|币安OTC|otc价格|查询币安价格)$/i, async (ctx) => {
  try {
    ...
    while (retryCount <= maxRetries && (!buyData || !sellData)) {
      if (retryCount > 0) {
        // 等待1秒后重试
      ...
      if (results[0].status === 'fulfilled') buyData = results[0].value
      if (results[1].status === 'fulfilled') sellData = results[1].value
      
      retryCount++
    }
    
    // 🔥 如果仍然失败，尝试使用备用数据源（实时汇率）
    if (!buyData && !sellData) {
